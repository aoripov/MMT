...
abstract class ThesaurusAbstractFormatter(name : String) extends Presenter(new InformalMathMLPresenter) {
...
class ThesaurusFormatter extends ThesaurusAbstractFormatter("thesaurus") {

   def setRh(rh : RenderingHandler) = this._rh = rh
   override val logPrefix = "ThesaurusFormatter"
