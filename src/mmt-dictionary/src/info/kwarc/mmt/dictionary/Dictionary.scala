...
//  private var indexTree: DictionaryTrie = new DictionaryTrie()
//  private
//}
abstract class DictionaryGenericBase[A] {
  protected var indexTree : DictionaryTrie[A] = new DictionaryTrie[A]
  protected var indexed : Boolean = false

  def addTranslation(from: String, to: String, term: String, data: A) : Unit = {
    this.indexTree.append(from, to, term, data)
  }

  def addTranslations(translations: Map[String,Map[String,Map[String, A]]]) = {
    for ((from, toMap) <- translations) {
      for ((to, terms) <- toMap) {
        for ((term, trans) <- terms) {
          
          this.indexTree.append(from, to, term, trans)
        }
      }
    }
  }

  def findTranslations(from: String, to: String, q: String) : Option[A] = {
    this.indexTree.findTranslation(from, to, q)
  }

  def findByPrefix(from: String, to: String, prefix: String) : Iterable[A] = {
    this.indexTree.findByPrefix(from, to, prefix)
  }

  def clearIndex(): Unit = {
    this.indexTree = new DictionaryTrie[A]
  }

  def getAvailableLanguages(): Unit = {
    this.indexTree.getAvailableLanguages
  }
}

class SMGloMDictionary extends  DictionaryGenericBase[(GlobalName, List[String], String)] {
  ...
  def getPairsOfLanguage() = {
    ...
    val tmp1 = paths map (x => controller.getO(x))
      val tmp2 = tmp1.collect {
        case Some(x) => x
      }
    //val tmp = (paths map (x => controller.get(x))).toList

    val constants = tmp2.collect {
        case c: Constant =>
          //addNotation("en",c)
  ...
  def getDefLinks() : JSONObject = {
    val paths = controller.depstore.getInds(ontology.IsConstant)
    val constants = paths.map(x => controller.get(x)).toList map(t => t match {
      case c: Constant =>
    ...
    val temps = constants.flatten.toMap
//      .toList.collect(x => x match{
//      case c: Constant => {
//        c.notC.verbalizationDim.notations.values.toList.flatten map {n =>
//          clean(toStringMethod(n)) -> c.path.toPath
//        }
//      }
//    }).flatten.toMap
    new JSONObject(temps)
  }

  def loadIndex() = {
    println(this.indexed)
    if (!this.indexed) {
      for ((g, tr) <- this.consLang) {
        val langs = tr.keys.toList

        for (from <- langs) {
          for (to <- langs) {
            val nots = tr.get(from).get
            for (n1 <- nots) {
              val n2 = tr.get(to).get
              this.indexTree.append(from, to, toStringMethod(n1), (g, n2.map(toStringMethod),toStringMethod(n1)))
            }
          }
        }
      }
...
object SMGloMDictionary extends SMGloMDictionary